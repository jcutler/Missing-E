...
function moreAnswerOptions(item, tagAsker, defTags, betterAnswers) {
   ...
   if (!lang) { lang = 'en'; }
   var id = MissingE.escapeHTML(jQuery(item).attr('id').match(/[0-9]*$/)[0]);
   if (betterAnswers === 1) {
      var allbtns = "";
      var suffix;
      ...
      jQuery(item).find('input.MissingE_askFixes_tags').keydown(function(e) {
...
self.on('message', function (message) {
   ...
   if (message.askDash === 1) {
      ...
      jQuery('#posts div.user_menu_list a[href$="/ask"]')
            .live('click', function() {
         var user = MissingE.escapeHTML(jQuery(this).closest('div.user_menu_list')
