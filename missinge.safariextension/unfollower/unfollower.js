...
function tu_init() {
   $("body").append('<div id="113977_unfollowdisplay" style="display:none;"><div style="font:bold 24px Georgia,serif;color:#1f354c;">unfollowr.</div><div class="unfollowerlist" style="height:' + ((getPageHeight()/10)*7) + 'px;overflow-y:auto;text-align:center;margin-top:10px;"></div><img class="logo" src="' + safari.extension.baseURI + 'Icon-64.png' + '" /></div>');
...
function doGet(num, show) {
   ...
   if (show) {
      $('#113977_unfollowdisplay .unfollowerlist').html('<p><img src="' + safari.extension.baseURI + 'facebox/loading.gif' + '" /></p>');
      ...
      $.ajax({
         ...
         error: function(xhr, textStatus) {
            ...
            if (!failed && this.tryCount <= this.retryLimit) {
               ...
               failed = true;
               $('#113977_unfollowdisplay .unfollowerlist').html('<p><em>Having trouble getting followers listing from Tumblr\'s servers, please try again later.</em></p><img style="margin:20px 0;" src="' + safari.extension.baseURI + 'images/oh_dear.png' + '" /><div><em>Artwork by <a href="http://theoatmeal.com/">The Oatmeal</a></em></div>');
