...
function addAskReblog(item) {
   ...
   if (item.tagName === "LI" && $(item).hasClass('post') &&
      ...
      chrome.extension.sendRequest({greeting: "betterReblogs", pid: tid,
                                    url: perm.attr("href")},
                                   function(response) {
   ...
   if (reblog_settings.quickReblog === 1) {
      ...
      $('head').append('<style type="text/css">' +
         ...
         '.MissingE_quick_reblogging_success {' +
            'background-image:url("' +
            chrome.extension.getURL('betterReblogs/reblog_success.png') +
            '") !important; }</style>');
      ...
      if (list.length > 0) {
         ...
         list.each(function(i) {
            ...
            if ((reblog_settings.accountName === 0 &&
                 i === 0) ||
                reblog_settings.accountName === acct) {
               sel = true;
            }
      ...
      qr.find('#MissingE_quick_reblog_selector select').click(function(e) {
         ...
         changeQuickReblogAcct($(this),
                               reblog_settings.quickReblogForceTwitter);
      });
      ...
      $('#posts div.post_controls a[href^="/reblog/"]')
         ...
         var account = reblog_settings.accountName;
      ...
      else if (reblog_settings.quickReblogForceTwitter === "on") {
