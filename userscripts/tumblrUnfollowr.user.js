...
var fbcss = '#facebox{position:absolute;top:0;left:0;z-index:100;text-align:left;} #facebox .fbpopup{position:relative;border:3px solid rgba(0,0,0,0);-webkit-border-radius:5px;-moz-border-radius:5px;border-radius:5px;-webkit-box-shadow:0 0 18px rgba(0,0,0,0.4);-moz-box-shadow:0 0 18px rgba(0,0,0,0.4);box-shadow:0 0 18px rgba(0,0,0,0.4);} #facebox .fbcontent{display:table;width:370px;padding:10px;background:#fff;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;} #facebox .fbcontent > p:first-child{margin-top:0;} #facebox .fbcontent > p:last-child{margin-bottom:0;} #facebox .fbclose{position:absolute;top:5px;right:5px;padding:2px;background:#fff;} #facebox .fbclose img{opacity:0.3;} #facebox .fbclose:hover img{opacity:1.0;} #facebox .fbloading{text-align:center;} #facebox .fbimage{text-align:center;} #facebox img{border:0;margin:0;} #facebox_overlay{position:fixed;top:0px;left:0px;height:100%;width:100%;} .facebox_hide{z-index:-100;} .facebox_overlayBG{background-color:#000;z-index:99;} .facebox p{margin:10px 0;}';

var tucss = '#unfollowrtable { width:80%; margin:0 10%; border-collapse:separate; } #unfollowrtable .tu-firstrow { border-top:1px solid #444444; } #unfollowrtable .tu-lastrow { border-bottom:1px solid #444444; } #unfollowrtable a { text-decoration:none; } #unfollowrtable a:hover { font-style:italic !important; } #unfollowrtable td { width:100%; } #unfollowrtable .tu-greyrow { background-color:#DDDDDD; }'
...
function tu_init() {
   jQ("body").append('<div id="113977_unfollowdisplay" style="display:none;"><div style="font:bold 24px Georgia,serif;color:#1f354c;">unfollowr.</div><div class="unfollowerlist" style="height:' + ((getPageHeight()/10)*7) + 'px;overflow-y:auto;text-align:center;margin-top:10px;"></div></div>');
   ...
   var deltxt = '<a id="113977_unfollowdelta" class="tracked_tag_control" onclick="return false;" href="#">&Delta;</a>';
   var fw = jQ("#113977_followwhonotin");
   if (fw.size()>0) {
      fw.before(deltxt);
   }
   else {
   ...
   jQ('#113977_unfollowdelta').click(function() {
   ...
   jQ(document).ajaxError(function(e, xhr, settings) {
      if (!failed && /\/followers\/page\/[0-9]*/.test(settings.url)) {
         failed = true;
         jQ('#113977_unfollowdisplay .unfollowerlist').html('<p><em>There was an error, please try again later.</em></p>');
         jQ.facebox({ div: '#113977_unfollowdisplay' }, 'unfollowrbox');
      }
   });
}
...
function doGet(num, show) {
   ...
   if (show) {
      jQ('#113977_unfollowdisplay .unfollowerlist').html('<p><img src="' + limg + '" /></p>');
      jQ.facebox({ div: '#113977_unfollowdisplay' }, 'unfollowrbox');
   }
...
function doFinish(newlist,show) {
   ...
   if (unfollows.length == 0)
      jQ('#113977_unfollowdisplay .unfollowerlist').html('<p><em>Nobody has unfollowed you.</em></p>');
   else {
      ...
      jQ('#113977_unfollowdisplay .unfollowerlist').html(txt);
   }
   jQ.facebox({ div: '#113977_unfollowdisplay' }, 'unfollowrbox');
   jQ('#113977_unfollowdisplay .unfollowerlist').empty();
   a = [];
   b = [];
   currlist = [];
   newlist = [];
